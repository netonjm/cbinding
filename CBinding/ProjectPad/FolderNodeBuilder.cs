...
namespace CBinding.ProjectPad
{
	...
	abstract class FolderCommandHandler : NodeCommandHandler
	{
		...
		public override bool CanDropNode (object dataObject, DragOperation operation)
		{
			...
			if (dataObject.GetType ().Name == "SolutionFolderFileNode") {
				return true;
				...
				//} else if (dataObject is Components.DragPasteboard pasteboard) {
				//	return pasteboard.Count > 0;
			}
			return false;
		}
		...
		//		string [] files = new string [pasteboard.Count];
		//		for (int n = 0; n < pasteboard.Count; ++n) {
		//			// Get the current file
		//			string path = pasteboard.GetItem (n);
		...
		public async void AddFilesToProject ()
		{
			...
			fdiag.TransientFor = IdeApp.Workbench.Window;
		...
		public async void AddFilesFromFolder ()
		{
			...
			var foundFiles = EnumerateAllNonHiddenFiles (srcRoot, SearchOption.AllDirectories);
			...
			//var controller = new IncludeNewFilesController (srcRoot);
			//controller.AddFiles (foundFiles);
			//if (controller.Show ()) {
			//	var srcFiles = controller.SelectedFiles;
			...
			//	var node = Controller.GetNodeAtObject (new ProjectFolder (targetRoot, project));
			//	if (node != null)
			//		node.Expanded = true;
			//}
		}
		...
		public async void AddExistingFolder ()
		{
			...
			//var foundFiles = EnumerateAllNonHiddenFiles (srcRoot, SearchOption.AllDirectories);
			//var controller = new IncludeNewFilesController (srcRoot.ParentDirectory);
			//controller.AddFiles (foundFiles);
			...
			//	var node = Controller.GetNodeAtObject (new ProjectFolder (selectedFolder, project));
			//	if (node != null)
			//		node.Expanded = true;
			//}
		}
		...
		/// </summary>
		static IEnumerable<string> EnumerateAllNonHiddenFiles (string path, SearchOption searchOption)
		{
			return Directory.EnumerateFiles (path, "*", searchOption)
				.Where (file => !IsHidden (file));
		}

		static bool IsHidden (FilePath path)
		{
		...
		public async void AddNewFolder ()
		{
			...
			//var curNode = CurrentNode.Clone ();
			//curNode.Expanded = true;
			//if (curNode.FindChild (new ProjectFolder (folder, project))) {
